...
func ReturnStuffs(layouts string, querys map[string]interface{}) {
	...
	returnFuncs.GetReturns(layoutReturn, "Mrp_po_engine_get_id")
...
func dbInserts(filesName []string) {
	...
	//ReturnInserts
	//returnInsertsInstance.HeaderInsert(layout, data)
	...
	GTJson(jsonData)
	ws.SendReturn(jsonData, "Mrp_po_upd_id")
